...
final translationProvider = Provider<String Function(String, {List<String>? args})>((ref) {
  final locale = ref.watch(localeProvider);
  
  // This provider will rebuild whenever locale changes
  return (String key, {List<String>? args}) {
    debugPrint('TranslationProvider called for key: $key in locale: ${locale.languageCode}');
...
final translationServiceProvider = FutureProvider<LocalizationService>((ref) async {
  final locale = ref.watch(localeProvider);
  
  // Ensure the localization service is loaded with the current locale
  await localizationService.load(locale);
  debugPrint('TranslationServiceProvider loaded for locale: ${locale.languageCode}');
