...
/// Specialized dropdown for selectable items with code and name
class CommonDropdownFieldWithIcon<T> extends StatelessWidget {
  const CommonDropdownFieldWithIcon({
    super.key,
    required this.items,
    required this.onChanged,
  ...
  /// Function to get icon widget from item
  final Widget Function(T) getItemIcon;

  @override
  Widget build(BuildContext context) {
    return CommonDropdownField<T>(
      items: items,
      value: value,
      onChanged: onChanged,
      hint: hint,
      ...
      displayStringForItem: getItemName,
      itemBuilder:  (context, item) {
        return Row(
          children: [
            SizedBox(
