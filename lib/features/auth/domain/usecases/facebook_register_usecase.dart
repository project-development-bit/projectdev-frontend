...
final facebookRegisterUseCaseProvider =
    Provider<FacebookRegisterUseCase>((ref) {
  return FacebookRegisterUseCase(ref.watch(authRepositoryProvider));
});

// Facebook Register Use Case
class FacebookRegisterUseCase
    implements UseCase<LoginResponseModel, FacebookRegisterRequest> {
  final AuthRepository repository;

  FacebookRegisterUseCase(this.repository);

  @override
  Future<Either<Failure, LoginResponseModel>> call(
      FacebookRegisterRequest request) async {
    return await repository.facebookRegister(request);
  }
}
