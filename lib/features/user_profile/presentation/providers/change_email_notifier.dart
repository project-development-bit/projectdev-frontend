import 'package:gigafaucet/features/user_profile/domain/usecases/change_email_usecase.dart';
...
class ChangeEmailState {
  ...
  ChangeEmailState copyWith(
      {ChangeEmailStatus? status, String? errorMessage, String? newEmail}) {
...
final changeEmailProvider =
    StateNotifierProvider<ChangeEmailNotifier, ChangeEmailState>((ref) {
...
class ChangeEmailNotifier extends StateNotifier<ChangeEmailState> {
  ...
  Future<void> changeEmail(
      ...
      required String repeatNewEmail}) async {
    state =
        state.copyWith(status: ChangeEmailStatus.changing, errorMessage: null);

    final result = await _usecase.call(ChangeEmailParams(
    ...
    result.fold((failure) {
      state = state.copyWith(
          ...
          errorMessage: failure.message ?? 'Failed to change email');
    }, (response) {
      state = state.copyWith(
          status: ChangeEmailStatus.success, newEmail: response.newEmail);
    });
  }
}
