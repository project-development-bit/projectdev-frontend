...
class VerifyEmailChangeState {
  ...
  VerifyEmailChangeState({this.status = VerifyEmailChangeStatus.initial, this.message});

  VerifyEmailChangeState copyWith({VerifyEmailChangeStatus? status, String? message}) {
    return VerifyEmailChangeState(status: status ?? this.status, message: message ?? this.message);
  }
}

final verifyEmailChangeProvider = StateNotifierProvider<VerifyEmailChangeNotifier, VerifyEmailChangeState>((ref) {
...
class VerifyEmailChangeNotifier extends StateNotifier<VerifyEmailChangeState> {
  ...
  Future<void> verifyEmailChange({required String email, required String code}) async {
    state = state.copyWith(status: VerifyEmailChangeStatus.verifying, message: null);

    final result = await _usecase.call(VerifyEmailChangeParams(email: email, code: code));

    result.fold((failure) {
      state = state.copyWith(status: VerifyEmailChangeStatus.failure, message: failure.message ?? 'Failed to verify email change');
    }, (response) {
      state = state.copyWith(status: VerifyEmailChangeStatus.success, message: response.message);
    });
  }
}
