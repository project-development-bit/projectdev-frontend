  ...
  group('Riverpod reCAPTCHA State Management Tests', () {
    ...
    group('RecaptchaNotifier Tests', () {
      ...
      test('should reset to appropriate state when reset is called', () {
        ...
        expect(container.read(recaptchaNotifierProvider),
            isA<RecaptchaVerified>());
    ...
    group('Consolidated Auth Provider Tests', () {
      test('should create consolidated state with reCAPTCHA state', () {
        ...
        expect(
            consolidatedState.recaptchaState,
            anyOf([
              isA<RecaptchaInitial>(),
              isA<RecaptchaNotAvailable>(),
            ]));
    ...
    group('Provider Integration Tests', () {
      test(
          'should maintain consistency between individual and consolidated providers',
          () {
        container = ProviderContainer();

        // Check individual providers match consolidated state
        final individualRecaptchaState =
            container.read(recaptchaNotifierProvider);
        final consolidatedState = container.read(consolidatedAuthStateProvider);

        expect(
            consolidatedState.recaptchaState, equals(individualRecaptchaState));

        final individualIsVerified =
            container.read(isRecaptchaVerifiedProvider);
        expect(consolidatedState.isRecaptchaVerified,
            equals(individualIsVerified));
      });
      ...
      test('should update consolidated state when reCAPTCHA state changes', () {
        ...
        final updatedConsolidatedState =
            container.read(consolidatedAuthStateProvider);
        expect(updatedConsolidatedState.isRecaptchaVerified, true);
        expect(
            updatedConsolidatedState.recaptchaState, isA<RecaptchaVerified>());
      });
    });

    group('State Transition Tests', () {
      test(
          'should transition through states correctly during verification flow',
          () {
        container = ProviderContainer();
        final notifier = container.read(recaptchaNotifierProvider.notifier);
      ...
      test('should handle error transitions correctly', () {
        ...
        expect(container.read(recaptchaErrorProvider),
            equals('Verification failed'));
      });
    });
  });
}
